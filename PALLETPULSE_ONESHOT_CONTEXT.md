...
### Trial UI Implementation

**Trial Visibility Pattern:**
Trial users should always know they're on a trial with clear indicators throughout the app.

**Key UI Components:**

1. **UserProfileCard (Settings Screen)**
   - Shows green "PRO TRIAL (X days)" badge next to user's name
   - Gift icon indicates trial status
   - Badge shows days remaining dynamically

2. **Trial Banner (Expenses Screen)**
   - Two-line banner at top of screen
   - Line 1: "Paid Feature" (bold)
   - Line 2: "X days left in trial" (lighter)
   - "Upgrade" button links to PaywallModal
   - Shows throughout entire trial duration

3. **"YOUR TRIAL" Section (Settings Screen)**
   - Replaces "SUBSCRIPTION" section during active trial
   - Card with gift icon, "Pro Trial Active" title
   - Shows days remaining
   - Explanation: "You have full access to Pro features. Subscribe before your trial ends to keep access."
   - "Subscribe Now" CTA button

**Trial-Aware Permission Pattern:**
```typescript
// Always compute effective tier considering trial status
const trialActive = isTrialActive();
const currentTier = trialActive ? 'pro' : getEffectiveTier();

// Check permissions against computed tier
const canAccessExpenses = currentTier !== 'free';
const canExportPDF = currentTier === 'pro' || currentTier === 'enterprise';
```

**Important:** Don't use `canPerform()` from subscription store for trial-sensitive features. The store's `canPerform` may not sync properly with trial state. Instead, compute the effective tier locally and check against it.

### Workflow 5: Analytics & Insights

**Trigger:** User wants to review business performance

**COGS Model for Period-Based Analytics:**

PalletPro follows the **Matching Principle** in accounting: expenses are recorded in the same period as the revenues they helped generate. This is implemented via the COGS (Cost of Goods Sold) model.

| Context | Cost Calculation | When Used |
|---------|------------------|-----------|
| **All-Time Views** | Full pallet cost | Pallet detail, item detail, inventory screens |
| **Period-Filtered Views** | Sum of `allocated_cost` for items sold in period | Dashboard (week/month/year), Analytics (date range) |

**Why COGS Matters:**
- Without COGS, viewing Q1 analytics would show -100% ROI if you bought a pallet in Q1 but haven't sold items yet
- COGS ensures profit/ROI only reflects costs for items actually sold in the selected period
- The `allocated_cost` field on each item represents its share of the pallet cost

**Implementation:**
```typescript
// For period-based analytics, use item-level allocated_cost
const cogs = soldItemsInPeriod.reduce((sum, item) =>
  sum + (item.allocated_cost ?? item.purchase_cost ?? 0), 0);

// For all-time pallet views, use full pallet cost
const totalCost = pallet.purchase_cost + pallet.sales_tax;
```

**Key Metrics (Dashboard):**
- **Total Profit:** All-time, this month, this week (uses COGS for period filtering)
...
- Trial banner on Dashboard (shows in last 3 days of trial for urgency)
- Trial banner on Expenses screen (shows throughout trial to indicate paid feature)
...
**6. Settings**
- **User Profile Card:** Shows user avatar, name, email, and subscription badge
  - During trial: Shows green "PRO TRIAL (X days)" badge with gift icon
  - After trial: Shows tier badge (FREE, STARTER, PRO)
- **Trial Section:** (only during active trial) Replaces Subscription section
  - "YOUR TRIAL" header
  - Card showing "Pro Trial Active", days remaining, explanation
  - "Subscribe Now" CTA button
- **Subscription section:** (only for paid subscribers, not during trial) Current plan with badge, tier description, feature list, Upgrade/Manage button, Restore Purchases
...
- **Developer section:** (dev mode only) Trial status display, Restart Trial button, Reset Onboarding
...
- ✅ CSV export (all data)
- ✅ COGS model for period-based analytics (Matching Principle compliance)
