...
### 11I: COGS Analytics Model ✅
**Completed:** Jan 24, 2026

**Overview:**
Implemented Cost of Goods Sold (COGS) model for period-based analytics to follow the Matching Principle in accounting. This fixes the -100% ROI bug that occurred when viewing future quarters (full costs counted but no revenue yet).

**Problem Solved:**
- Previously, analytics showed -100% ROI for future quarters because full pallet costs were counted but only period-filtered revenue was shown
- The Matching Principle requires expenses to be recorded in the same period as the revenues they helped generate

**COGS Model:**
| Metric | Traditional (All-Time) | COGS (Period-Based) |
|--------|------------------------|---------------------|
| Cost | Full pallet purchase cost | Sum of `allocated_cost` for items sold in period |
| Revenue | All sales ever | Sales within date range |
| Use Case | Pallet/item detail screens | Analytics with date filters |

**Files Modified:**
- `src/features/analytics/utils/analytics-calculations.ts` - Added COGS model
  - Added `calculateCOGS()` function for period-based cost calculation
  - Updated `calculateHeroMetrics()` to use COGS with date range
  - Updated `calculatePalletLeaderboard()` to use COGS with date range
  - Updated `calculateTypeComparison()` to use COGS with date range
  - Updated `calculateSupplierComparison()` to use COGS with date range
  - Updated `calculatePalletTypeComparison()` to use COGS with date range
...
| Screen | Date Filtering | Uses COGS | Notes |
|--------|----------------|-----------|-------|
| Dashboard | Yes (week/month/year/all) | ✅ Already correct | Uses `allocated_cost` |
| Analytics | Yes (date range) | ✅ Now implemented | Commit `e8ca99d` |
| Pallet Detail | No | N/A | All-time view, uses full cost |
| Item Detail | No | N/A | Uses `allocated_cost` for item profit |
| Inventory | No | N/A | All-time view, correct as-is |

**Unit Tests Added:**
- `calculateCOGS` unit tests (5 tests)
- COGS integration tests for all calculation functions
- Test for preventing false negative ROI for future quarters
- Fixed existing test fixture to use `allocated_cost: null`
...
**Commits:**
- `feat(analytics): implement COGS model for period-based analytics` (e8ca99d)

---

### 11J: Trial UI Improvements ✅
**Completed:** Jan 24, 2026

**Overview:**
Improved trial visibility and fixed permission checks so users clearly understand their trial status throughout the app. Fixed issues where trial users couldn't access expense tracking despite being on Pro trial.

*(Previously numbered 11I before COGS analytics was added)*

**Files Modified:**
- `app/(tabs)/_layout.tsx` - Trial-aware tab visibility for Expenses tab
- `app/(tabs)/expenses.tsx` - Trial banner component, trial-aware permission checks
- `app/(tabs)/settings.tsx` - Trial section card, trial-aware checks, developer options
- `src/components/subscription/PaywallModal.tsx` - Improved free tier display, better continue button
- `src/features/settings/components/UserProfileCard.tsx` - PRO TRIAL badge with days remaining

**Key Features:**

1. **PRO TRIAL Badge in UserProfileCard**
   - Shows green "PRO TRIAL (X days)" badge next to user's name
   - Gift icon indicates trial status
   - Dynamically shows days remaining

2. **Trial Banner on Expenses Screen**
   - Two-line banner: "Paid Feature" / "X days left in trial"
   - Upgrade button links to PaywallModal
   - Shows throughout trial duration (not just last 3 days)

3. **"YOUR TRIAL" Section in Settings**
   - Replaces misleading "SUBSCRIPTION" section during trial
   - Shows "Pro Trial Active" with days remaining
   - Clear explanation of trial benefits
   - "Subscribe Now" CTA button

4. **Trial-Aware Permission Checks**
   - Fixed pattern: `currentTier = trialActive ? 'pro' : getEffectiveTier()`
   - Expenses tab now visible during trial
   - Expense tracking toggle works during trial

5. **Improved PaywallModal**
   - Added Free tier card showing current plan for free users
   - Shows what free tier lacks (no expense tracking, no mileage, etc.)
   - Better "Continue with Free" / "Stay on Free Plan" button

6. **Developer Options**
   - "Trial Status" row shows current trial state
   - "Restart Trial (7 days)" button for testing

**Bug Fixes:**
- Trial users couldn't enable expense tracking (showed paywall) - FIXED
- Expenses tab not showing in navigation during trial - FIXED
- Trial banner text was truncating ("7 day" instead of "7 days") - FIXED
- Settings showed misleading "Current Plan: Pro" during trial - FIXED
...
**Commits:**
- `feat(trial): improve trial UI with clear indicators and permission fixes`
